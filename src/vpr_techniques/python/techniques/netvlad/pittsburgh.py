...
root_dir = os.getcwd().replace('src/vpr_techniques/python/techniques/netvlad', '') + 'src/data/raw_data/Pittsburgh250k'
...
def collate_fn(batch):
    ...
    negCounts = data.dataloader.default_collate([int(x.shape[0]) for x in negatives])
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, nNegSample=1000, nNeg=10, margin=0.1, input_transform=None):
        ...
        self.cache = 'tmpdir/train_feat_cache.hdf5'  # filepath of HDF5 containing feature vectors for images
