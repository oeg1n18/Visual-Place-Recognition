...
def get_query_paths(session_type='ms', rootdir=None):
    if isdir(rootdir + '/src/data/raw_data/SFU'):
        return sorted(glob.glob(rootdir + '/src/data/raw_data/SFU/jan/*.jpg'))
    else:
        print("must download dataset first into rootdir + /src/data/raw_data/ directory")


def get_map_paths(session_type='ms', rootdir=None):
    if isdir(rootdir + '/src/data/raw_data/SFU'):
        return sorted(glob.glob(rootdir + '/src/data/raw_data/SFU/dry/*.jpg'))
    else:
        print("must download dataset first into rootdir + /src/data/raw_data/ directory")

def get_gtmatrix(session_type='ms', gt_type='hard', rootdir=None):
    gt_data = np.load(rootdir + '/src/data/raw_data/SFU/GT.npz')
    if gt_type=='hard':
...
def download(rootdir=None):
    destination = rootdir + '/src/data/raw_data/SFU'
