...
def getROIs(imgConvFeat, imgLocalConvFeat, img):
    ...
    for ROI in range(len(allROI_Box)):
        ...
        aggregatedNROIs[ROI, :] = np.sum(
            imgLocalConvFeat[allROI_Box[ROI][0]:allROI_Box[ROI][2], allROI_Box[ROI][1]:allROI_Box[ROI][3]], axis=(0,
                                                                                                                  1))  # Sometimes elements come out as nan for rare images, which breaks the code. I have picked this up with the author and for the time being patched this. Maybe some issue with package versions. Needs further debugging. #Mubariz
