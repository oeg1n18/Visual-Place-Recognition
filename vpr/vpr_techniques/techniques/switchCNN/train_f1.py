...
from vpr.vpr_techniques.techniques.switchCNN.models import resnet9regressionModule
...
class accuracyDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        data = pd.read_csv(config.root_dir + '/vpr/vpr_techniques/techniques/switchCNN/data/f1_dataset.csv')
...
if __name__ == '__main__':
    ...
    model = resnet9regressionModule()
    checkpoint_callback = ModelCheckpoint(dirpath="weights/",
                                          filename='resnet9-f1-{epoch}-{val_loss:.2f}-{other_metric:.2f}',
