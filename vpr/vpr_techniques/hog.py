...
def compute_query_desc(Q, dataset_name=None, disable_pbar=False):
    ...
    ref_desc_list = []
    for batch in tqdm(np.array_split(np.array(Q), int(max(len(Q)/config.batch_size, 1))), desc='Computing Query Descriptors', disable=disable_pbar):
        ref_batch = [cv2.imread(pth, 0) for pth in batch]
...
def compute_map_features(M, dataset_name=None, disable_pbar=False):
    ...
    hog = cv2.HOGDescriptor(winSize, blockSize, blockStride, cellSize, nbins)
    ref_desc_list = []
    for batch in tqdm(np.array_split(np.array(M), int(max(len(M)/config.batch_size, 1))), desc='Computing Map Descriptors', disable=disable_pbar):
        ref_batch = [cv2.imread(pth, 0) for pth in batch]
        for ref_image in ref_batch:
            if ref_image is not None:
                hog_desc = hog.compute(cv2.resize(ref_image, winSize))
